...
from tests.helpers import dict_to_link_params, dict_to_node_params, add_node
...
class TestRedisMongoDB:
    ...
    @pytest.mark.parametrize("node",[
        ("A", "type_a", "redis_mongo_db", {"status": "ready"})
    ])
    def test_get_atoms_by_index(self, node, database: RedisMongoDB):
        node = add_node(database, *node)
        result = database.create_field_index("node", fields=["custom_attributes.status"])
        cursor, actual = database.get_atoms_by_index(result, [{"field": "custom_attributes.status", "value": "ready"}])
        assert cursor == 0
        assert isinstance(actual, list)
        assert len(actual) == 1
